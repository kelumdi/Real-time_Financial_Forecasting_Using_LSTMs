...
subplot(3,3,2);
A1 = csvread('trueoil.csv');
A2 = csvread('predictedTest_commoil_mimicKalmanModified.csv');
A3 = csvread('EKF_oil.csv');
A4 = csvread('predictedTest_oil_AR.csv');
A5 = csvread('predictedTest_oil_arima.csv');
% x = linspace(1,30,30);
% x1 = A1(1259-29:1259,1);
x = linspace(1,199,199);
x1 = A1(8448-198:8448,1); %eleminate last one day
x2 = A2(1:199,1);%LSTM : eleminate last one day
x3 = A3(1:199,1); %EKF : eliminate last one day
x4 = A4(2:200,1); %AR : eliminate first 1day
x5 = A5(2:200,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot(3,3,4);
A = csvread('truegas.csv');
x = A(:,1); %eleminate last one day
plot(x,'m','LineWidth',1)
xlabel({'(d)'})
ylabel({'Price($)','','Natural Gas'})
xline(5952-149,'--r');
box off;
set(gca,'FontWeight','bold','FontSize',12)
hold off
...
% hold off
subplot(3,3,5);
A1 = csvread('truegas.csv');
A2 = csvread('predictedTest_commgas_mimicKalmanModified.csv');
A3 = csvread('EKF_gas.csv');
A4 = csvread('predictedTest_gas_AR.csv');
A5 = csvread('predictedTest_gas_arima.csv');
x = linspace(1,149,149);
x1 = A1(5952-148:5952,1); %eleminate last one day
x2 = A2(1:149,1);%LSTM : eleminate last one day
x3 = A3(1:149,1); %EKF : eliminate last one day
x4 = A4(2:150,1); %AR : eliminate first 1day
x5 = A5(2:150,1);
plot(x,x1,'m', x,x2,'b', x,x3,'r',x,x4,'g',x,x5,'c','LineWidth',1)
%legend('True','LSTM','Kalman','AR','ARIMA')
xlabel('(e)')
...
A2 = csvread('predictedTest_commgold_mimicKalmanModified.csv');
A3 = csvread('EKF_gold.csv');
A4 = csvread('predictedTest_gold_AR.csv');
A5 = csvread('predictedTest_gold_arima.csv');
x = linspace(1,29,29);
x1 = A1(846-28:846,1); %eleminate last one day
