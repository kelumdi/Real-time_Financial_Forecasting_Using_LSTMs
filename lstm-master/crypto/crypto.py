...
def getDataTrainTest_csvTrue(pathStock, normalize, ss, winInput, numPredic):
    ...
    # call function to  get trainInput, trainLabel, testInput, testLabel by window-based-data : train 1064days & test 1day
...
# get trainInput, trainLabel, testInput, testLabel by window-based-data : train 1064days & test 1day
...
def trainLSTM3MSE(normalize, ss, model,
    ...
    for epoch in range(1, n_epochs + 1): #start epoch
        ...
        if epoch == 1:
            ...
            testL0 = outputTestL2[0]  # get scaler from a list
...
def runDayByDay(trainInputL2, trainLabelL2, testInputL2, testLabelL2, shuffleTrain,
                batchSizeTrain, batchSizeTest, input_size, hidden_size, nLayer, output_size,
                dropout1, normalize, ss,  n_epochs, lrRnnAdam, seqLengthInput,
                seqLengthLabel):
    # get train data (trainInput,trainLabel) and test data (testInput,testLabel)
    train_xy = getData(trainInputL2, trainLabelL2)
    test_xy = getData(testInputL2, testLabelL2)  # 3585list w/ tuple w/ [16,2]
...
def main():
    winInput = 1064 #input size for one batch
    winLabel = 1  #label size for one batch
    shuffleTrain = False  #We do not shuffle

    coin = 'BTC' #name of our cryptocurrency
    ...
    # get trainInput, trainLabel,testInput,testLabel for each cryptocurrency
    if coin == 'BTC':
        stock1 = './data/BTC.csv'
        ss1 = StandardScaler() #standardize
        (trainInputL, trainLabelL, testInputL, testLabelL, ss) = getDataTrainTest_csvTrue(stock1,
                                                                                                          normalize,
                                                                                                          ss1, winInput,
                                                                                                          numPredic)
    elif coin == 'ETH':
    ...
    for j in range(len(testInputL) - 1):
        ...
        (trainInputL[j + 1][1063], testInputL[j + 1][1063]) = runDayByDay(trainInputL2, trainLabelL2, testInputL2,testLabelL2,shuffleTrain,
                                                                          batchSizeTrain,batchSizeTest, input_size,
                                                                          hidden_size, nLayer, output_size,
                                                                          dropout1, normalize, ss, n_epochs,
                                                                          lrRnnAdam, seqLengthInput, seqLengthLabel)

        outWinL.append(testInputL[j + 1][1063])  #save the prediction of each test day
    ...
    (predicLastDayTrain, predicLastDayTest) =  runDayByDay(trainInputL2, trainLabelL2, testInputL2,testLabelL2,shuffleTrain,
                                                                          batchSizeTrain,batchSizeTest, input_size,
                                                                          hidden_size, nLayer, output_size,
                                                                          dropout1, normalize, ss, n_epochs,
                                                                          lrRnnAdam, seqLengthInput, seqLengthLabel)
    predicLastDayTest2 = predicLastDayTest.reshape(predicLastDayTest.shape[0], )
    outWinL2 = outWinL + [predicLastDayTest2]
    outWin4 = np.array(outWinL2)

    # save the predicted 30 day
    np.savetxt(
        './out2/predictedTest_stock{}_mimicKalmanModified_{}winInput{}winLabel_{}epoch.csv'.format(
            coin, winInput, winLabel, n_epochs), outWin4, delimiter=",")
